...
The R software for running `GDCRNATools` can be downloaded from [The Comprehensive R Archive Network (CRAN)](https://cran.r-project.org/). The `GDCRNATools` package can be installed from [Bioconductor](https://www.bioconductor.org/).

```{r installation3, eval=TRUE, message=FALSE, warning=FALSE}
...
# Quick start
To perform the comprehensive data analysis, some functions are built for users to download and process GDC data efficiently.
...
Here we use a small dataset from TCGA-CHOL project to show the most basic steps for ceRNA network analysis. A more detailed instruction of each step is followed.
...
```{r parse meta3 q, message=FALSE, warning=FALSE, eval=TRUE}
...
datatable(as.data.frame(metaMatrix.MIR[1:5,]), extensions = 'Scroller',
          options = list(scrollX = TRUE, 
                         deferRender = TRUE,
                         scroller = TRUE))
```
...
datatable(as.data.frame(deLNC), 
          options = list(scrollX = TRUE, pageLength = 5))

#### DE protein coding genes
dePC <- gdcDEReport(deg = DEGAll, gene.type = 'protein_coding')
datatable(as.data.frame(dePC), 
          options = list(scrollX = TRUE, pageLength = 5))
```
...
```{r edges, message=FALSE, warning=FALSE, eval=TRUE}
...
```{r nodes, message=FALSE, warning=FALSE, eval=TRUE}
...
# Case study: TCGA-PRAD
In this section, we use data from the TCGA-PRAD project as an example to illustrate how the `GDCRNATools` works in detail.
...
Step3: Download data using `gdcRNADownload()` and `gdcClinicalDownload()` functions by providing the manifest file 

### Automatic download  
To provide users a more convenient method for data download, by default, we used the API method developed in the `GenomicDataCommons` package to download data automatically. An alternative method using the gdc-client for automatic download is also provided in case that the API method fails.
...
testdir <- tempdir()
rnadir <- paste(rnadir, 'RNAseq', sep='/')
mirdir <- paste(rnadir, 'miRNAs', sep='/')
...
The `shinyKMPlot()` function is also a simply `shiny` app which allow users view KM plots (based on the R package [survminer](https://cran.r-project.org/web/packages/survminer/index.html).) of all genes of interests on a local webpackage conveniently.
