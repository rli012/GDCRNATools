...
##' @description Bar plot and bubble plot for GO, KEGG, and DO functional enrichment analysis
##' @param enrichment a dataframe generated from \code{\link{gdcEnrichAnalysis}}
##' @param type type of the plot, should be one of \code{'bar'} and \code{'bubble'}
##' @param category which category should be plotted. Possible values are 
##'   \code{'KEGG'}, \code{'GO'}, \code{'GO_BP'}, \code{'GO_CC'}, \code{'GO_MF'}, and \code{'DO'}.
...
gdcEnrichPlot <- function(enrichment, type='bar', category='KEGG', num.terms=10, bar.color='black') {
...
enrichBubblePlotFun <- function(kegg, pval=0.01) {
  keggBasic = ggplot(data=kegg, mapping=aes(x=Terms, y=foldEnrichment,color=FDR,size=Counts))
  keggBasic+geom_point()+ coord_flip() + scale_x_discrete(limits=rev(kegg$Terms)) + 
    scale_colour_gradientn(limits=c(0,pval),colors= c("red","yellow","green")) + xlab('')+ylab('Fold enrichment') +
    ...
    theme(strip.text = element_text(size = 12), legend.key.size = unit(0.8,'cm'))
...
enrichBarPlotFun <- function(kegg, type='single', bar.color='black') {
  ...
  if (type=='single') {
    ...
    keggBasic + geom_bar(stat='identity',fill=bar.color) + scale_x_discrete(limits=rev(kegg$Terms)) + 
      ylim(0, max(-log(kegg$FDR,10))) + theme(legend.title=element_blank())+ylab('-log10(FDR)')+
      xlab('') + coord_flip() + 
      theme(axis.text.y=element_text(size=12), axis.title=element_text(size=15),
            axis.text.x=element_text(size=12)) + 
    ...
    keggBasic = ggplot(data=kegg, mapping=aes(x=Terms, y=-log(FDR,10), fill=Category))
    keggBasic + geom_bar(stat='identity') + scale_x_discrete(limits=rev(kegg$Terms)) + 
      ylim(0, max(-log(kegg$FDR,10))) + theme(legend.title=element_blank())+ylab('-log10(FDR)')+
      xlab('') + coord_flip() + 
