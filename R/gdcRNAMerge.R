...
##' @return A dataframe or numeric matrix of raw counts data with rows are genes or miRNAs and columns are samples
##' @export
...
gdcRNAMerge <- function(metadata, path, data.type) {
  ...
  if (data.type=='RNAseq') {
    ...
    rownames(rnaMatrix) <- unlist(lapply(strsplit(rownames(rnaMatrix), '.', fixed=T), function(gene) gene[1]))
...
cleanMirFun <- function(fl) {
  expr <- read.table(fl, header=T, stringsAsFactors = F)
  expr <- expr[startsWith(expr$miRNA_region, "mature"),]
  expr <- aggregate(expr$read_count, list(expr$miRNA_region), sum)
  
  mirs <- unlist(lapply(strsplit(expr$Group.1, ',', fixed=T), function(mir) mir[2]))
