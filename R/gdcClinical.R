...
##'   will be downloaded via gdc-client, otherwise, \code{project} argument should be provided to download 
...
##' @param key.info logical, whether to return the key clinical information only. If \code{TRUE}, only clinical
...
gdcClinicalMerge <- function(path, key.info=TRUE) {
  ...
  filenames <- file.path(folders, unlist(lapply(folders, function(v) getXMLFile(v))), fsep = .Platform$file.sep)
  ...
  if (key.info== TRUE){
    ...
    cleantable <- data.frame(t(cleantable), stringsAsFactors = F)
    
    rownames(cleantable) <- gsub('.', '-', rownames(cleantable), fixed=T)
...
parseXMLFun <- function(fl) {
  ...
  names(test1) <- make.names(unlist(test2), unique = T)
