...
##' @description Performs Gene Ontology (GO), Kyoto Encyclopedia of Genes and Genomes (KEGG) pathway 
##'   and Disease Ontology (DO) enrichment analyses by \pkg{clusterProfiler} and \pkg{DOSE} packages
##' @param gene a vector of Ensembl gene id
##' @param simplify logical, specifying whether to remove redundant GO terms. Default \code{simplify=TRUE}
##' @param level a numeric value, restrict the GO enrichment result at a specific GO level. Default is \code{0}, 
##'   which means all terms should be returned
...
##' @return A dataframe of enrichment analysis result containing enriched terms, 
##'   number of overlpped genes, p value of hypergeometric test, fdr, fold of enrichment, Ensembl gene ids,
##'   gene symbols, and functional categories, etc.
##' @references
##'   Yu G, Wang LG, Han Y, He QY. clusterProfiler: an R package for comparing biological themes among gene clusters. Omics: a journal of integrative biology. 2012 May 1;16(5):284-7. \cr
...
gdcEnrichAnalysis <- function(gene, simplify=TRUE, level=0) {
  ...
  genes <- getBM(attributes = c('ensembl_gene_id','entrezgene'), values=gene, filters='ensembl_gene_id', mart=ensembl)
  genes <- genes[! is.na(genes$entrezgene),]
  universe <- getBM(attributes = c('ensembl_gene_id','entrezgene'), values=biotype$ensemblID, filters='ensembl_gene_id', mart=ensembl)
  ...
  kegg$geneID <- unlist(lapply(kegg$geneID, function(v) 
    paste(genes$ensembl_gene_id[match(strsplit(v, '/', fixed=T)[[1]], genes$entrezgene)], collapse = '/')))
  ...
  enrichOutput$Category <- rep(c('GO_BP','GO_CC','GO_MF','KEGG', 'DO'), c(nrow(goBP),nrow(goCC),nrow(goMF),nrow(kegg),nrow(do)))
...
organizeEnrichFun <- function(go) {
  ...
  geneID <- go$geneID
  geneSymbol <- unlist(lapply(strsplit(geneID, '/', fixed=T), function(v) paste(ensembl2symbolFun(v), collapse = '/')))
  
  goOutput <- data.frame(Terms, Counts, GeneRatio, BgRatio, pValue, FDR, foldEnrichment, geneID, geneSymbol)
